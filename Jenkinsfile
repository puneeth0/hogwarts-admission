pipeline {
    ...
    stages{
        stage('Build Maven'){
            steps{
                checkout([$class: 'GitSCM', branches: [[name: '*/master']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/puneeth0/hogwarts-admission']]])
        ...
        stage('Build docker image'){
            steps{
                script{
                    bat 'docker build -t puneeth0/hogwarts-admission .'
                }
            }
        }
        stage('Push docker image to Hub'){
            steps{
                script{
                    ...
                    bat 'docker push puneeth0/hogwarts-admission'
                    }
                }
            }
        stage('Deploy to k8s'){
